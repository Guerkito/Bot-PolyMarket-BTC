...
class BinanceFeed:
    """Feed de precio en tiempo real vía WebSocket de Binance (aggTrade) con fallback HTTP."""
...
class CryptoBotElite:
    ...
    # ── Apuesta ─────────────────────────────────────────────────────────────────
    def place_bet(self, engine, side):
        ...
        if not MODO_SIMULACION and self.real_ready:
            ...
            limit_price = price # para el historial
    ...
    def run(self):
        while True:
            ...
            # Lógica de apuesta con todos los filtros
            if not is_paused:
                for eng in [self.btc, self.eth]:
                    if BET_WINDOW_END <= eng.seconds_left <= BET_WINDOW_START and not eng.bet_placed:
                        up, down = eng.prices["UP"], eng.prices["DOWN"]
                        side = None
                        max_price = ETH_MAX_ENTRY_PRICE if eng.asset == "ETH" else 0.97
                        if THRESHOLD <= up < max_price and eng.has_momentum("UP"):
                            side = "UP"
                        elif THRESHOLD <= down < max_price and eng.has_momentum("DOWN"):
                            side = "DOWN"
                        if side:
                            if self.place_bet(eng, side):
